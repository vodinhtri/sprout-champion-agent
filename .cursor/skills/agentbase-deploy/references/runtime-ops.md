...
All operations use the runtime script: `bash .cursor/skills/agentbase/scripts/runtime.sh`
...
bash .cursor/skills/agentbase/scripts/vserver.sh projects

# 2. List VPCs in the project — filtered to id, name, cidr, status, dnsStatus
bash .cursor/skills/agentbase/scripts/vserver.sh vpcs <projectId>
...
bash .cursor/skills/agentbase/scripts/vserver.sh validate-vpc <projectId> <vpcId>
```
...
```bash
bash .cursor/skills/agentbase/scripts/runtime.sh list --page 1 --size 20
```
...
bash .cursor/skills/agentbase/scripts/runtime.sh endpoints delete $RUNTIME_ID $CUSTOM_ENDPOINT_ID

# 2. Then delete the runtime
bash .cursor/skills/agentbase/scripts/runtime.sh delete $RUNTIME_ID
```
...
**Create endpoint**:
```bash
bash .cursor/skills/agentbase/scripts/runtime.sh endpoints create $RUNTIME_ID \
  --name canary --version 2
```

**Update endpoint version**:
```bash
bash .cursor/skills/agentbase/scripts/runtime.sh endpoints update $RUNTIME_ID $ENDPOINT_ID \
  --version 3
```

**Delete endpoint**:
```bash
bash .cursor/skills/agentbase/scripts/runtime.sh endpoints delete $RUNTIME_ID $ENDPOINT_ID
```

**Start / stop endpoint** (lifecycle control — stop halts the endpoint from serving traffic without deleting it; start resumes it):
```bash
bash .cursor/skills/agentbase/scripts/runtime.sh endpoints stop  $RUNTIME_ID $ENDPOINT_ID
bash .cursor/skills/agentbase/scripts/runtime.sh endpoints start $RUNTIME_ID $ENDPOINT_ID
```
...
```bash
bash .cursor/skills/agentbase/scripts/runtime.sh versions $RUNTIME_ID --page 1 --size 20
```
...
while true; do
  STATUS=$(bash .cursor/skills/agentbase/scripts/runtime.sh get $RUNTIME_ID | jq -r '.status')
  echo "Status: $STATUS"
...
```bash
bash .cursor/skills/agentbase/scripts/runtime.sh reset-service-account $RUNTIME_ID
```
...
```bash
bash .cursor/skills/agentbase/scripts/runtime.sh flavors
```
